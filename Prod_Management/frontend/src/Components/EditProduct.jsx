...
const EditProduct = () => {
    ...
    useEffect(()=>{
      axios.get(`http://localhost:3333/prod/fetchsingle/${id}`)
      .then((resp)=>setProduct({...resp.data, quantity: resp.data.quantity || 0}))
      .catch((err)=>console.log(err))
    },{id});
  
    const updateProduct = () => {
      axios
        .put(`http://localhost:3333/prod/updateprod/${id}`,product)
    ...
    <div className="container d-flex justify-content-center align-items-center" style={{ minHeight: '70vh' }}>
      <div style={{ minWidth: '350px', maxWidth: '700px', margin: 'auto', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', borderRadius: '12px', background: '#f8f9fa', padding: '24px', border: '1px solid #dee2e6' }}>
        ...
        <form className="d-flex flex-column align-items-center w-100">
          ...
          <button type="button" className="btn btn-dark w-100 mt-2" onClick={updateProduct}>
            Update Product
          </button>
        </form>
      </div>
